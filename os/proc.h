...
// Project 2: Forward declaration for page table type
typedef uint64 *pagetable_t;  // Page table is a pointer to page table entries
...
struct proc {
	...
	// Project 2: Virtual memory support
	// - pagetable: Maps virtual addresses to physical addresses for this process
	// - max_page: Highest virtual page allocated (for memory tracking)
	pagetable_t pagetable;  // Project 2: Page table for virtual memory
	uint64 max_page;        // Project 2: Highest virtual page number allocated
	
	// Project 2: Time tracking
	// - start_time: When process first ran (in milliseconds), -1 if never ran
	// - syscall_times: Array of counters tracking calls per syscall ID
	int start_time;         // Project 2: Time when process first started (ms)
	unsigned int syscall_times[MAX_SYSCALL_NUM];  // Project 2: Per-syscall counters
};
...
void freeproc(struct proc *p);                      // Project 2: Free process resources
